...
// ============================================================================
// DG tet P2 SIPG Poisson operator.
// ============================================================================
...
struct DgTetP2DofMap {
    std::string resolvedSpace = "dg_tet_p2";
    int nCells = 0;
    int nDofs = 0;
};

struct DgTetP2CellGeom {
...
static DgTetP2DofMap build_dg_tet_p2_dof_map(const PolyMesh& m) {
    MeshCellCounts cc = count_cell_types(m);
    if (cc.nTet != (int)m.cells.size() || cc.nHex != 0 || cc.nOther != 0) {
        throw std::runtime_error("dg_tet_p2 requires a pure tet mesh.");
    }
...
static DgTetP2CellGeom dg_tet_p2_build_geom(const PolyMesh& m, const Cell& cell) {
    if (cell.verts.size() != 4) {
        throw std::runtime_error("DG tet P2 found non-tet cell.");
    }
    ...
    if (!(K.detJabs > 0.0)) {
        throw std::runtime_error("DG tet P2 found degenerate tet.");
    }
    K.invJ = inv3(K.J);
...
static std::vector<DgTetP2CellGeom> build_dg_tet_p2_geoms(const PolyMesh& m) {
    std::vector<DgTetP2CellGeom> geoms(m.cells.size());
    for (int c = 0; c < (int)m.cells.size(); ++c) {
        geoms[c] = dg_tet_p2_build_geom(m, m.cells[c]);
    }
    return geoms;
}

static int dg_tet_p2_local_index_for_vertex(const DgTetP2CellGeom& K, int v) {
    for (int a = 0; a < 4; ++a) {
...
static TetP2BasisAtPoint dg_tet_p2_face_basis(
    const Face& f,
    const DgTetP2CellGeom& K,
    double lam0,
    double lam1,
    double lam2
) {
    if (f.verts.size() != 3) {
        throw std::runtime_error("DG tet P2 expected triangular face.");
    }
    ...
    for (int q = 0; q < 3; ++q) {
        const int li = dg_tet_p2_local_index_for_vertex(K, f.verts[q]);
        if (li < 0) {
            throw std::runtime_error("DG tet P2 face vertex not found in cell.");
        }
        L[li] = lf[q];
    }
...
static Vec3 dg_tet_p2_owner_outward_unit_normal(
    const PolyMesh& m,
    int faceId,
    const DgTetP2CellGeom& ownerGeom,
    double& area
) {
    ...
    if (!(mag > 0.0)) {
        throw std::runtime_error("DG tet P2 degenerate triangular face.");
    }
...
static Vec3 dg_tet_p2_node_coordinate(const DgTetP2CellGeom& K, int a) {
    if (a < 4) return K.x[a];
...
static std::vector<std::vector<int>> build_dg_tet_p2_sipg_pattern(
    const PolyMesh& m,
    const DgTetP2DofMap& dm
) {
...
static AssembledSystem assemble_dg_tet_p2_sipg_poisson_mms(
    const PolyMesh& m,
    const DgTetP2DofMap& dm,
    ...
    if (sparseMode == "fixed_csr") {
        sparse_init_fixed_pattern(sys.A, build_dg_tet_p2_sipg_pattern(m, dm));
    } else if (sparseMode == "legacy") {
        sys.A.rows.resize(dm.nDofs);
    } else {
        throw std::runtime_error("Unsupported DG tet P2 sparse mode: " + sparseMode);
    }

    const auto geom = build_dg_tet_p2_geoms(m);
    ...
    for (int f = 0; f < (int)m.faces.size(); ++f) {
        ...
        double area = 0.0;
        const Vec3 n = dg_tet_p2_owner_outward_unit_normal(m, f, KP, area);
        ...
        for (const auto& fq : triangle_face_quadrature_selected()) {
            ...
            const TetP2BasisAtPoint phiP = dg_tet_p2_face_basis(m.faces[f], KP, lam0, lam1, lam2);
            ...
            if (interior) {
                const TetP2BasisAtPoint phiN = dg_tet_p2_face_basis(m.faces[f], *KN, lam0, lam1, lam2);
...
static StrongDirichletData build_dg_tet_p2_boundary_strong_mms(
    const PolyMesh& m,
    const DgTetP2DofMap& dm,
    const std::string& mms
) {
    ...
    const auto geom = build_dg_tet_p2_geoms(m);
    std::vector<char> mark(10 * (int)m.cells.size(), 0);
    StrongDirichletData bc;

    // Local edge dof order in eval_tet_p2_basis_at.
    static const int edgeLut[4][4] = {
        {-1, 4, 5, 6},
        { 4,-1, 7, 8},
        { 5, 7,-1, 9},
        { 6, 8, 9,-1}
    ...
    auto add_dof = [&](int dof, const Vec3& x) {
        if (!mark[dof]) {
            mark[dof] = 1;
            bc.dofs.push_back(dof);
            bc.values.push_back(Real(mms_exact_value(x, mms)));
        }
    };

    for (int f = (int)m.neighbour.size(); f < (int)m.faces.size(); ++f) {
        const int c = m.owner[f];
        const DgTetP2CellGeom& K = geom[c];
        int lv[3] = {-1, -1, -1};
        for (int q = 0; q < 3; ++q) {
            lv[q] = dg_tet_p2_local_index_for_vertex(K, m.faces[f].verts[q]);
            if (lv[q] < 0) throw std::runtime_error("DG tet P2 strong BC face vertex not found.");
            add_dof(10*c + lv[q], K.x[lv[q]]);
        }
        for (int a = 0; a < 3; ++a) {
            for (int b = a + 1; b < 3; ++b) {
                const int ed = edgeLut[lv[a]][lv[b]];
                if (ed < 0) throw std::runtime_error("DG tet P2 strong BC invalid local edge.");
                add_dof(10*c + ed, dg_tet_p2_node_coordinate(K, ed));
            }
        }
    }
...
static AssembledSystem assemble_dg_tet_p2_strong_poisson_mms(
    const PolyMesh& m,
    const DgTetP2DofMap& dm,
    const std::string& mms,
    const DgSipgOptions& dgopt,
    ...
    AssembledSystem sys = assemble_dg_tet_p2_sipg_poisson_mms(m, dm, mms, dgopt, spec, false);
    StrongDirichletData bc = build_dg_tet_p2_boundary_strong_mms(m, dm, mms);
    apply_strong_dirichlet_by_dof_values(bc, sys.A, sys.b);
    return sys;
}

static ErrorNorms compute_dg_tet_p2_error_norms(
    const PolyMesh& m,
    const DgTetP2DofMap& dm,
    const std::string& mms,
    ...
    if ((int)x.size() != dm.nDofs) {
        throw std::runtime_error("DG tet P2 error norm size mismatch.");
    }
    ...
    for (int c = 0; c < dm.nCells; ++c) {
        ...
        for (int a = 0; a < 10; ++a) {
            ...
            const Vec3 xa = dg_tet_p2_node_coordinate(K, a);
            const double eNode = ua - mms_exact_value(xa, mms);
            nodal2 += eNode * eNode;
...
static void probe_dg_tet_p2_system(
    const PolyMesh& m,
    const DgTetP2DofMap& dm,
    const AssembledSystem& sys,
    int diagLevel
) {
    std::cout << "---------------- DG tet P2 SIPG system probe -----------------\n";
