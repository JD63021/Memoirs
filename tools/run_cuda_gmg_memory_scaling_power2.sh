...
OUTDIR="$PROJECT/logs/cuda_gmg_memory_scaling_power2_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTDIR"

SUMMARY="$OUTDIR/summary.csv"
echo "prec,N,status,it,finalRel,checkedRel,xL2,baselineMiB,peakMiB,netPeakMiB,coarseDofs,log" > "$SUMMARY"
...
run_case() {
  ...
  echo
  echo "============================================================"
  echo "CUDA GMG $prec N=$N, nested power-of-two, coarseMaxDofs=27"
  echo "============================================================"
  ...
  local it finalRel checkedRel xL2 base peak net cdofs
  ...
  cdofs="$(extract_field gmgCudaCoarseDofs "$log")"

  echo "$prec,$N,$status,${it:-NA},${finalRel:-NA},${checkedRel:-NA},${xL2:-NA},${base:-NA},${peak:-NA},${net:-NA},${cdofs:-NA},$log" >> "$SUMMARY"

  echo "RESULT prec=$prec N=$N status=$status it=${it:-NA} coarseDofs=${cdofs:-NA} netPeakMiB=${net:-NA}"
  sleep 3
}
