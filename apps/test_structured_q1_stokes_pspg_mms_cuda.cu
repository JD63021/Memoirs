...
int main(int argc, char** argv) {
    ...
    try {
        ...
        memoirs::gpu::DeviceBuffer<Real> d_exact(static_cast<std::size_t>(total));

        A.fill_exact(d_exact.data());
        if (cli.rhsMode == 0) {
            A.assemble_rhs_continuous(d_b.data());
        } else {
            A.apply(d_exact.data(), d_b.data());
        }
        ...
        std::cout << "solver                    = right_" << cli.solver << "\n";
        std::cout << "gmresRestart              = " << cli.gmresRestart << "\n";
        std::cout << "rightPreconditioner       = inverse_diagonal\n";
        std::cout << "rhsMode                   = "
                  << (cli.rhsMode == 0 ? "continuous_quadrature" : "algebraic_Ax_exact") << "\n";
        std::cout << "nu                        = " << cli.nu << "\n";
        ...
        if (cli.rhsMode == 0) {
            memoirs::gpu::print_exact_rhs_defect_local<Real>(
                A, d_b.data(), d_exact.data(), grid.n_nodes(), std::cout);
        }

        M.print_audit(std::cout);
        ...
        for (int r = 0; r < cli.repeats; ++r) {
            ...
            timer.start();
            if (cli.solver == "gmres") {
                rep = memoirs::gpu::gmres_right_prec_cuda_local<Real>(
                    A, M, d_b.data(), d_x.data(),
                    Real(cli.tol), cli.maxit, cli.gmresRestart, cli.printEvery);
            } else {
                rep = memoirs::gpu::bicgstab_right_prec_cuda_local<Real>(
                    A, M, d_b.data(), d_x.data(),
            ...
            const double checkedRel = memoirs::gpu::device_residual_rel_local(A, d_b.data(), d_x.data());

            std::vector<Real> h_x(static_cast<std::size_t>(total));
            d_x.copy_to_host(h_x.data(), h_x.size());

            const auto err =
                memoirs::diagnostics::structured_q1_l2_error_stokes_mms<Real>(grid, h_x);
            ...
            std::cout << "breakdownValue            = " << std::setprecision(17) << rep.breakdownValue << "\n";
            ...
            std::cout << "checkedRelativeResidual   = " << checkedRel << "\n";
