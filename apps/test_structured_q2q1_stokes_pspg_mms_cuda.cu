...
struct Cli {
    ...
    int printEvery = 100;
    int repeats = 1;
    int rhsMode = 0; // 0 continuous, 1 algebraic A*x_exact
...
static Cli parse_cli(int argc, char** argv) {
    ...
    if (c.solver != "gmres") throw std::runtime_error("Q2/Q1 v1 supports -solver gmres only");
    return c;
}
...
template <class RealT>
__global__ void kernel_apply_q2q1_elements(Q2Q1GridCuda g, RealT nu, RealT tau, const RealT* x, RealT* y) {
    ...
    for (int iq=0; iq<4; ++iq) {
        ...
        for (int jq=0; jq<4; ++jq) {
            ...
            for (int kq=0; kq<4; ++kq) {
                ...
                // Momentum rows: nu*(grad v, grad u) - (p, div v).
                for (int a=0; a<27; ++a) if (!vbnd[a]) {
                    const int row = vgid[a];
                    const double ku = nu * (dVx[a]*gradUx[0] + dVy[a]*gradUx[1] + dVz[a]*gradUx[2]) - ph*dVx[a];
                    const double kv = nu * (dVx[a]*gradUy[0] + dVy[a]*gradUy[1] + dVz[a]*gradUy[2]) - ph*dVy[a];
                    const double kw = nu * (dVx[a]*gradUz[0] + dVy[a]*gradUz[1] + dVz[a]*gradUz[2]) - ph*dVz[a];
                    atomicAdd(&y[row], RealT(wq*ku));
                    atomicAdd(&y[g.nv + row], RealT(wq*kv));
                    atomicAdd(&y[2*g.nv + row], RealT(wq*kw));
                }

                // Pressure rows: (q, div u) + tau*(grad q, -nu*Delta u + grad p).
                for (int a=0; a<8; ++a) if (pgid[a] != pPin) {
                    const double r0 = Np[a]*divU + double(tau)*(dPx[a]*(-double(nu)*lapUx + gradP[0])
                                                              + dPy[a]*(-double(nu)*lapUy + gradP[1])
                                                              + dPz[a]*(-double(nu)*lapUz + gradP[2]));
...
template <class RealT>
__global__ void kernel_assemble_rhs_q2q1(Q2Q1GridCuda g, RealT nu, RealT tau, RealT* rhs) {
    const int e = blockIdx.x * blockDim.x + threadIdx.x;
    const int nElem = g.N*g.N*g.N;
    if (e >= nElem) return;
    int ex,ey,ez; elem_from_linear(g,e,ex,ey,ez);
    ...
    for (int iq=0; iq<4; ++iq) {
        ...
        for (int jq=0; jq<4; ++jq) {
            ...
            for (int kq=0; kq<4; ++kq) {
                ...
                const double wq = qw[iq]*qw[jq]*qw[kq]*jac;
                double fx,fy,fz; stokes_force(xx,yy,zz,double(nu),fx,fy,fz);
                ...
                for (int az=0; az<3; ++az) for (int ay=0; ay<3; ++ay) for (int ax=0; ax<3; ++ax) {
                    ...
                    if (!v_boundary(g,gi,gj,gk)) {
                        ...
                        atomicAdd(&rhs[row], RealT(wq*Na*fx));
                        atomicAdd(&rhs[g.nv + row], RealT(wq*Na*fy));
                        atomicAdd(&rhs[2*g.nv + row], RealT(wq*Na*fz));
                    }
                }
                for (int az=0; az<2; ++az) for (int ay=0; ay<2; ++ay) for (int ax=0; ax<2; ++ax) {
                    const int a = q1_lid(ax,ay,az);
...
template <class RealT>
class Q2Q1StokesOperator {
public:
    Q2Q1StokesOperator(Q2Q1GridCuda g, RealT nu, RealT tau) : g_(g), nu_(nu), tau_(tau) {}
    ...
    void apply(const RealT* x, RealT* y) const {
        ...
        kernel_apply_q2q1_elements<RealT><<<div_up_local(g_.N*g_.N*g_.N,block),block>>>(g_,nu_,tau_,x,y);
        MEMOIRS_CUDA_KERNEL_CHECK();
        kernel_apply_q2q1_strong_rows<RealT><<<div_up_local(std::max(g_.nv,g_.np),256),256>>>(g_,x,y);
        MEMOIRS_CUDA_KERNEL_CHECK();
    }
    void assemble_rhs(RealT* rhs) const {
        const int block=128;
        kernel_zero_local<RealT><<<div_up_local(g_.ndof,256),256>>>(g_.ndof,rhs);
        MEMOIRS_CUDA_KERNEL_CHECK();
        kernel_assemble_rhs_q2q1<RealT><<<div_up_local(g_.N*g_.N*g_.N,block),block>>>(g_,nu_,tau_,rhs);
        MEMOIRS_CUDA_KERNEL_CHECK();
...
class Q2Q1DiagPrec {
public:
    Q2Q1DiagPrec(Q2Q1GridCuda g, RealT nu, RealT tau) : g_(g), nu_(nu), tau_(tau) {}
    void apply(const RealT* r, RealT* z) {
        kernel_diag_prec_q2q1<RealT><<<div_up_local(std::max(g_.nv,g_.np),256),256>>>(g_,nu_,tau_,r,z);
        MEMOIRS_CUDA_KERNEL_CHECK();
    }
private:
    Q2Q1GridCuda g_;
...
SolverReport gmres_right_prec(const Operator& A, RightPreconditioner& M, const RealT* d_b, RealT* d_x,
    ...
    DeviceBuffer<RealT> d_Ax(n), d_r(n), d_w(n);
    ...
    std::vector<DeviceBuffer<RealT>> V; V.reserve(restart+1);
    std::vector<DeviceBuffer<RealT>> Z; Z.reserve(restart);
    for(int i=0;i<restart+1;++i) V.emplace_back(n);
    for(int i=0;i<restart;++i) Z.emplace_back(n);
    std::vector<std::vector<double>> H(restart+1,std::vector<double>(restart,0.0));
    std::vector<double> cs(restart,0.0), sn(restart,0.0), g(restart+1,0.0), y(restart,0.0);
...
int main(int argc, char** argv) {
    try {
        ...
        const Real tau = Real(memoirs::gpu::compute_tau(g, cli.nu, cli.tauScale, cli.tauMode, cli.tauC, cli.tauDt, cli.tauAdvMag));
        using Op = memoirs::gpu::Q2Q1StokesOperator<Real>;
        using Prec = memoirs::gpu::Q2Q1BlockGmgPrec<Real>;
        Op A(g, Real(cli.nu), tau);
        Prec M(g, Real(cli.nu), tau);

        memoirs::gpu::DeviceBuffer<Real> d_x(g.ndof), d_b(g.ndof), d_exact(g.ndof);
        d_x.zero();
        const int block=256;
        ...
        else A.assemble_rhs(d_b.data());

        std::cout << "Memoirs CUDA structured Q2/Q1 Stokes PSPG MMS development test\n";
        ...
        std::cout << "operator                  = monolithic_q2q1_stokes_pspg_matrix_free_v1\n";
        std::cout << "mms                       = divergence_free_sine_velocity_meanzero_cos_sin_sin_pressure\n";
        ...
        std::cout << "rightPreconditioner       = block_scalar_gmg_velocity_q2grid_pressure_q1grid\n";
        std::cout << "rhsMode                   = " << (cli.rhsMode==0?"continuous_quadrature":"algebraic_Ax_exact") << "\n";
        std::cout << "nu                        = " << cli.nu << "\n";
        ...
        for (int r=0; r<cli.repeats; ++r) {
            ...
            rep = memoirs::gpu::gmres_right_prec<Real>(A,M,d_b.data(),d_x.data(),Real(cli.tol),cli.maxit,cli.gmresRestart,cli.printEvery);
            const auto t1=std::chrono::high_resolution_clock::now();
            totalSeconds += std::chrono::duration<double>(t1-t0).count();
        }
        const double checked = memoirs::gpu::checked_rel_residual<Real>(A,d_b.data(),d_x.data());
        ...
        std::cout << "--------------- q2q1 stokes pspg solve report ---------------\n";
        ...
        std::cout << "checkedRelativeResidual   = " << checked << "\n";
        ...
        std::cout << "--------------------------------------------------------------\n";
        std::cout << "solveSeconds              = " << totalSeconds / double(cli.repeats) << "\n";
        return (rep.converged && checked <= cli.tol*10.0) ? 0 : 2;
