...
#include "memoirs/sections/13_dg_hex_q1_sipg.hpp"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Memoirs v0 :: DG hex Q1 nodal SIPG Poisson MMS\n";
        std::cout << "precision         = " << kPrecisionName << "\n";
        std::cout << "polyMeshDir       = " << opt.polyMeshDir << "\n";
        std::cout << "space             = dg_hex_q1\n";
        ...
        if (opt.assemble || opt.solve) {
            ...
            if (scalarSpec.boundary.mode == ScalarBoundaryMode::Strong) {
                sys = assemble_dg_hex_q1_strong_poisson_mms(mesh, dm, opt.mms, dgopt);
            } else if (scalarSpec.boundary.mode == ScalarBoundaryMode::WeakSipg ||
                       scalarSpec.boundary.mode == ScalarBoundaryMode::WeakNitsche) {
                sys = assemble_dg_hex_q1_sipg_poisson_mms(mesh, dm, opt.mms, dgopt);
            } else {
                throw std::runtime_error("DG hex app supports -bc sipg or -bc strong.");
            }
            ...
            if (opt.diagLevel >= 1 || opt.assemble) {
                probe_dg_hex_q1_system(mesh, dm, sys, opt.diagLevel);
            }
            ...
            if (opt.solve) {
                ...
                ErrorNorms en = compute_dg_hex_q1_error_norms(mesh, dm, opt.mms, x);
